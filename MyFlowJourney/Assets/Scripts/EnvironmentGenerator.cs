...
public class EnvironmentGenerator : MonoBehaviour
{
	...
	private void OnValidate()
	{
		...
		_spriteShapeController.spline.InsertPointAt(_levelEnd, new Vector3(_lastPos.x, transform.position.y - _bottom));
		_spriteShapeController.spline.InsertPointAt(_levelEnd + 1, new Vector3(transform.position.x, transform.position.y - _bottom));
	}
}
